...
namespace DaleranGames.World
{
    ...
    public class MapData
    {
        ...
        public MapData(int columns, int rows, MapTemplate template, int level)
        {
            ...
            LayoutEnemies(template.enemies, enemyCount, Columns/2);
        }
        ...
        void LayoutEnemies (GameObject[] enemies, int count, float rangeFromEntrance)
        {
            List<Vector2Int> enemyPositions = gridPositions;

            for (int i = 0; i < enemyPositions.Count; i++)
            {
                float range = ((Vector2)enemyPositions[i] - (Vector2)EntrancePosition).magnitude;
                if (range < rangeFromEntrance)
            ...
            for (int j = 0; j < count; j++)
            {
                ...
                Vector2Int randomPosition = enemyPositions[randomIndex];

                for (int i = 0; i < enemyPositions.Count; i++)
                {
                    if (enemyPositions[i] == randomPosition)
                        enemyPositions.RemoveAt(i);
                }

                RemoveGridPosition(randomPosition);

                GameObject tileChoice = enemies[Random.Range(0, enemies.Length)];
                ObjectTiles[randomPosition.x, randomPosition.y] = tileChoice; 
            }
